...
// 应用：
// 内部函数引用外部函数的变量 + 内部函数向外暴露 = 内部函数只要不销毁，就可以持续访问到外部函数的变量，让数据持久化，但又不至于到全局，引起全局命名冲突

// 解决的问题：
// 局部变量无法共享和长久的保存，而全局变量回造成变量污染，当我们希望有一种机制即可以长久的保存变量又不会造成全局的污染。
